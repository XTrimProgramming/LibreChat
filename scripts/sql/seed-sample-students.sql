-- Auto-init script for the sample_students database used by MCP Postgres tests
-- Runs from the default postgres database on container startup.
\set ON_ERROR_STOP on

-- Create the sample database only if it does not already exist.
SELECT 'CREATE DATABASE sample_students'
WHERE NOT EXISTS (SELECT FROM pg_database WHERE datname = 'sample_students')
\gexec

\connect sample_students
